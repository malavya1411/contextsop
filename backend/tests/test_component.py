...
def test_generate_component_success(mock_get, mock_get_llm, client, auth_headers):
    mock_client = MagicMock()
    mock_get_llm.return_value = (mock_client, "gpt-4o-mini")
    ...
    res = client.post(
        "/api/v1/component/generate", json={"prompt": "Log colorizer widget"}, headers=auth_headers
    )
...
def test_generate_component_validation_error(mock_get, client, auth_headers):
    ...
    res = client.post("/api/v1/component/generate", json={"prompt": "ab"}, headers=auth_headers)
