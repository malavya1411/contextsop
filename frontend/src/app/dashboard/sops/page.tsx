  ...
  useEffect(() => {
    async function loadSops() {
      ...
      try {
        ...
        setError(
          "Unable to retrieve Standard Operating Procedures. Please try again.",
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-sidebar-border/40">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            Browse, manage, and execute your organization's interactive incident
            runbooks.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSops.map((sop) => {
            ...
            const updatedDate = new Date(sop.updated_at).toLocaleDateString(
              "en-US",
              {
                month: "short",
                day: "numeric",
              ...
              >
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <p className="text-sm text-text-muted line-clamp-3 leading-relaxed">
                    {sop.description ||
                      "No description provided. Run this procedure to configure parameter overrides and execute interactive steps."}
                  </p>
                </div>
